...
class BaseModelMerger(ABC):
    ...
    def get_transformers_auto_model_class(self):
        ...
        if archs:
            ...
            raise NotImplementedError(f"Unknown architecture {archs}")

        # architectures field missing or None: try to infer from model_type, else default
        model_type = getattr(self.model_config, "model_type", None)
        if model_type and "vision" in model_type:
            return AutoModelForVision2Seq

        warnings.warn(
            "Model config has no 'architectures' entry; defaulting to AutoModelForCausalLM. "
            "If this is incorrect, provide a Hugging Face config with 'architectures'."
        )
        return AutoModelForCausalLM
    ...
    def save_hf_model_and_tokenizer(self, state_dict: dict[str, torch.Tensor]):
        ...
        try:
            ...
            warnings.warn(f"Failed to create tokenizer via hf_tokenizer: {e}. Trying slow-tokenizer fallback.")
            try:
                tokenizer = AutoTokenizer.from_pretrained(self.hf_model_config_path, use_fast=False)
            except Exception as e2:
                tokenizer = None
                warnings.warn(f"Slow-tokenizer fallback failed: {e2}. Skipping tokenizer save.")
        ...
        if tokenizer is not None:
            try:
                ...
                warnings.warn(f"Failed to save tokenizer: {e}. Continuing without tokenizer.")
