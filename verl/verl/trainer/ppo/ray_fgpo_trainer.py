                ...
                with marked_timer("step", timing_raw):
                    ...
                    #########################################                    
                        if self.config.actor_rollout_ref.actor.use_refine:
                            ...
                            feedback_improved_mask = (feedback_reward > rewards_tensor).float()
                            ...
                            response_mask = feedback_batch.batch["response_mask"]
                            responses = feedback_batch.batch["responses"]
                            sft_input_ids = torch.cat([batch.batch["input_ids"].to(device), responses.to(device)], dim=1)
                            sft_attention_mask = torch.cat([batch.batch["attention_mask"].to(device), response_mask.to(device)], dim=1)
                            response_labels = responses.to(device).clone()
                            response_labels = response_labels.masked_fill(response_mask.to(device) == 0, -100)
                            sft_labels = torch.cat(
                                [torch.full_like(batch.batch["input_ids"], -100).to(device), response_labels],
                                dim=1,
                            )
                            prompt_position_ids = batch.batch["position_ids"].to(device)
                            prompt_lengths = batch.batch["attention_mask"].to(device).sum(dim=-1, keepdim=True).long()
                            response_position_ids = compute_position_id_with_mask(response_mask.to(device))
                            response_position_ids = response_position_ids + prompt_lengths

                            if prompt_position_ids.dim() == 2:
                                # Text-only: [B, S_prompt] + [B, S_resp] -> [B, S_prompt + S_resp]
                                sft_position_ids = torch.cat([prompt_position_ids.long(), response_position_ids.long()], dim=1)
                            elif prompt_position_ids.dim() == 3:
                                # Multimodal/mRoPE: [B, C, S_prompt] + [B, C, S_resp] -> [B, C, S_prompt + S_resp]
                                channels = prompt_position_ids.size(1)
                                response_position_ids = response_position_ids.unsqueeze(1).expand(-1, channels, -1)
                        ...
                        if self.config.actor_rollout_ref.actor.use_sft:
                            ...
                            sft_attention_mask = torch.cat(
                                [
                                    batch.batch["attention_mask"].to(device),
                                    sft_response_mask,
                                ],
                                dim=1,
                            )
                            ...
                            sft_labels = torch.cat(
                                [
                                    torch.full_like(batch.batch["input_ids"].to(device), -100),
                                    response_labels,
                                ],
                                dim=1,
                            )
                            ...
                            response_position_ids = compute_position_id_with_mask(
                                sft_response_mask
                            )
                            response_position_ids = response_position_ids + prompt_lengths
                            ...
                            if prompt_position_ids.dim() == 2:
                                ...
                                # Qwen2-VL / mRoPE:
                                # [B, C, S_prompt] + [B, C, S_resp]
                                channels = prompt_position_ids.size(1)
                                response_position_ids = response_position_ids.unsqueeze(1).expand(
                                    -1,
                                    channels,
                                    -1,
                                )
