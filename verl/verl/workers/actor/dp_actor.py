...
class DataParallelPPOActor(BasePPOActor):
    ...
    def __init__(self, config: ActorConfig, actor_module: nn.Module, actor_optimizer: torch.optim.Optimizer = None):
        ...
        # feedback
        self.success_reward_threshold = self.config["success_reward_threshold"]
        self.reprompt_truncation = self.config["reprompt_truncation"]
        self.dont_reprompt_on_self_success = self.config["dont_reprompt_on_self_success"]
        self.remove_thinking_from_demonstration = self.config["remove_thinking_from_demonstration"]
        self.use_refine = self.config["use_refine"]
        self.use_sft = self.config["use_sft"]
        self.reprompt_template = self.config["reprompt_template"]
        self.solution_template = self.config["solution_template"]
        self.feedback_template = self.config["feedback_template"]
        self.include_environment_feedback = self.config["include_environment_feedback"]
        self.max_reprompt_len = self.config["max_reprompt_len"]
        self.refine_coef = self.config["refine_coef"]
    def _forward_micro_batch(
        ...
        if "multi_modal_inputs" in micro_batch.keys():
            ...
            # print('micro_batch["multi_modal_inputs"]', micro_batch["multi_modal_inputs"])
        ...
        with torch.autocast(device_type=self.device_name, dtype=torch.bfloat16):
            ...
            if self.use_remove_padding:
                ...
                if self.use_fused_kernels:
                    ...
                    if calculate_entropy:
                        if not self.config.entropy_checkpointing:
                            entropy = verl_F.entropy_from_logits(response_logits)  # (bsz, response_length)
                        else:
                            entropy = torch.utils.checkpoint.checkpoint(verl_F.entropy_from_logits, response_logits)
    ...
    def update_policy(self, data: DataProto):
        ...
        if self.use_refine or self.use_sft:
            refine_keys = [
                            "sft_input_ids",
                            "sft_attention_mask",
                            "sft_labels",
            ...
            select_keys.extend([k for k in refine_keys if k in data.batch])
        ...
        for _ in range(self.config.ppo_epochs):
            for batch_idx, mini_batch in enumerate(mini_batches):
                ...
                for micro_batch in micro_batches:
                    ...
                    raw_reward_tensor = model_inputs.get("raw_reward", None)
                    if raw_reward_tensor is None:
                        raise KeyError("model_inputs is missing 'raw_reward'")
                    raw_reward_tensor = raw_reward_tensor.view(-1)  # [B, 1] or [B]
                    ...
                    if self.use_dpo:
                        ...
                        with torch.autocast(device_type=self.device_name, dtype=torch.bfloat16):
                            ch_out = self.actor_module(
                                input_ids=ch_ids, attention_mask=ch_mask, 
                                # position_ids=ch_pos,
                                **mm_inputs, use_cache=False, **extra_args, #return_dict=True,
                            )
                            rj_out = self.actor_module(
                                input_ids=rj_ids, attention_mask=rj_mask, 
                                # position_ids=rj_pos,
                                **mm_inputs, use_cache=False, **extra_args, #return_dict=True,
                            )
                            # print("ch_out", ch_out)
                        ...
                        if self.use_fused_kernels:
                            ...
                            # print("ch_lbls", (ch_lbls== -100).sum(dim=1), (ch_lbls != -100).sum(dim=1))
                            # print("rj_lbls", (rj_lbls== -100).sum(dim=1), (rj_lbls != -100).sum(dim=1))
                        ...
                        rj_log_probs_reuse = (log_prob * response_mask).sum(dim=-1)
                        with torch.no_grad():
                            max_diff = (rj_log_probs_reuse.detach() - rj_log_probs.detach()).abs().max()
                        if torch.distributed.get_rank() == 0:
                            logger.info(
                                f"[DPO DEBUG] max |rj_log_probs (reuse - recompute)| = "
                                f"{max_diff.item():.6g}"
                            )                        
                        # print("log_prob", log_prob.shape)
                        # print("rj_log_probs_reuse", rj_log_probs_reuse.shape)
                        
                        # print("ch_log_probs", ch_log_probs.shape)
                        # print("rj_log_probs", rj_log_probs.shape)
                        # diff = (rj_log_probs_reuse.detach() - rj_log_probs.detach())
                        # print("diff", diff)
                        ...
                        # else:                  
                            # print("dpo_loss!!!", dpo_loss)
                    if self.use_refine or  self.use_sft:
                        # Supervised fine-tuning on the chosen responses (prompt + chosen completion),
                        required_keys = [
                            "sft_input_ids",
                            "sft_attention_mask",
                            "sft_labels",
                        ...
                        missing_keys = [k for k in required_keys if k not in model_inputs]
                        if missing_keys:
                            raise KeyError(
                                f"use_refine=True or use_sft=True requires sft_* tensors, missing: {missing_keys}"
                            )

                        sft_input_ids = model_inputs["sft_input_ids"]
                        sft_attention_mask = model_inputs["sft_attention_mask"]
                        sft_labels = model_inputs["sft_labels"]
                        sft_position_ids = model_inputs.get("sft_position_ids", None)

                        # Handle Qwen2VL-style MRoPE position_ids if present
                        if sft_position_ids is not None:
                            if sft_position_ids.dim() == 3:
                                # (bsz, 4, seqlen) -> (4, bsz, seqlen)
                                sft_position_ids = sft_position_ids.transpose(0, 1).contiguous()
                            else:
                                sft_position_ids = sft_position_ids.contiguous()
                            sft_position_ids = sft_position_ids.to(torch.long)
                        ...
                        # Compute token-level SFT CE on labels (ignore -100)
                        if self.use_fused_kernels and hasattr(sft_out, "log_probs"):
                            # Fused kernels return per-token log probs directly.
                            # Enforce next-token semantics to match the non-fused CE path:
                            # target is labels[:, 1:], and log_probs should have length T-1.
                            token_logps = sft_out.log_probs
                            shift_labels = sft_labels[:, 1:]
                            if token_logps.shape == shift_labels.shape:
                                pass
                            elif token_logps.shape == sft_labels.shape:
                                token_logps = token_logps[:, :-1]
                            else:
                                raise ValueError(
                                    f"Unexpected fused log_probs shape {token_logps.shape} "
                                    f"for labels shape {sft_labels.shape}"
                                )

                            # valid supervised tokens mask (exclude prompt/pad masked by -100)
                            valid_token_mask = (shift_labels != -100).to(token_logps.dtype)

                            # token-level neg-log-prob as CE surrogate
                            token_ce = -token_logps  # (B, T')

                            # per-sequence average CE (so longer sequences don't dominate)
                            per_seq_denom = valid_token_mask.sum(dim=-1).clamp(min=1.0)   # [B]
                            per_seq_loss = (token_ce * valid_token_mask).sum(dim=-1) / per_seq_denom  # [B]

                            feedback_reward_tensor = model_inputs["feedback_reward"].to(per_seq_loss.dtype).view(-1)  # [B]
                            feedback_mask = model_inputs["feedback_mask"].to(per_seq_loss.dtype).view(-1)             # [B]

                            # sequence-level weight
                            # FIXME:
                            # seq_weight = feedback_reward_tensor * feedback_mask
                            seq_weight = feedback_mask
                            # weighted mean over sequences
                            weight_denom = seq_weight.sum() + 1e-8  # avoid div by zero
                            sft_loss = (per_seq_loss * seq_weight).sum() / weight_denom
                        else:
                            # Standard CE from logits
                            sft_logits = sft_out.logits
                            # sft_logits = sft_logits / temperature

                            shift_logits = sft_logits[..., :-1, :].contiguous()
                            shift_labels = sft_labels[..., 1:].contiguous().to(shift_logits.device)   # [B, T-1]

                            # token-level CE (flattened)
                            ...
                            # valid supervised tokens mask (exclude prompt/pad masked by -100)
                            valid_token_mask = (shift_labels != -100).to(token_ce.dtype)

                            # per-sequence average CE (so longer sequences don't dominate)
                            per_seq_denom = valid_token_mask.sum(dim=-1).clamp(min=1.0)   # [B]
                            per_seq_loss = (token_ce * valid_token_mask).sum(dim=-1) / per_seq_denom  # [B]
                            
                            feedback_reward_tensor = model_inputs["feedback_reward"].to(per_seq_loss.dtype).view(-1)  # [B, 1]
                            feedback_mask = model_inputs["feedback_mask"].to(per_seq_loss.dtype).view(-1)         # [B,1] or [B]
                            # sequence-level weight
                            # FIXME:
                            # seq_weight = feedback_reward_tensor * feedback_mask
                            seq_weight = feedback_mask
                            # Optional: avoid negative weights if feedback_reward can be negative
                            # weighted mean over sequences
                            weight_denom = seq_weight.sum() + 1e-8 
                            sft_loss = (per_seq_loss * seq_weight).sum() / weight_denom

                        if torch.isfinite(sft_loss).item():
                            loss = loss + self.refine_coef * sft_loss
                            print("sft_loss", sft_loss)
                        else:
                            logger.warning(f"Refine SFT loss is not finite: {sft_loss.item()}, skipping SFT loss for this batch.")
                        micro_batch_metrics["actor/refine_sft_loss"] = sft_loss.detach().item()
                        micro_batch_metrics["actor/refine_coef"] = self.refine_coef
                        micro_batch_metrics["actor/refine_ratio"] = feedback_mask.float().mean().item()
                        micro_batch_metrics["actor/refine_avg_reward"] = feedback_reward_tensor.mean().item()
                        micro_batch_metrics["actor/avg_reward"] = (feedback_reward_tensor.mean().item() + raw_reward_tensor.mean().item()) / 2
                    else:
                        micro_batch_metrics["actor/avg_reward"] =  raw_reward_tensor.mean().item()
