...
def _safe_to_text(value) -> str:
    """Best-effort conversion for debug logging; must never raise."""
    try:
        return str(value)
    except Exception:
        try:
...
def compute_score(solution_str: str, ground_truth: str, timeout_score: float = 0, prompt_str=None, data_source="unknown", format_weight: float = 0.1, **kwargs) -> bool:
    ...
    if os.getenv("DEBUG_MODE") == "true":
        ...
        safe_solution_extracted = _safe_to_text(solution_extracted)
        with open(log_path, "a", encoding='utf-8') as f:
            f.write(f"------------- {current_time} Accuracy reward: {accuracy_score} -------------\n")
            f.write(f"Prompt: {prompt_str}\n")
            f.write(f"Response: {solution_str}\n")
            f.write(f"Solution: {safe_solution_extracted}\n")
