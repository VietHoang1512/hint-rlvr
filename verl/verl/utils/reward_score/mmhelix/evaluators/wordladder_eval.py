...
class WordLadderEvaluator(BaseEvaluator):
    ...
    def _feedback(self, detailed: str, short: str) -> str:
        """Return detailed feedback unless it is too long."""
        return detailed if len(detailed) <= 100 else short
    ...
    def evaluate(
        ...
        if not pred_path or len(pred_path) < 2:
            return (
                False,
                self._feedback(
                    "Could not extract a valid Word Ladder path from the output. "
                    "Please provide at least a start word and a target word.",
                    "Invalid format: could not extract a valid Word Ladder path."
                )
            )
        ...
        if not start_word or not target_word:
            return False, self._feedback(
                "Missing puzzle metadata: could not determine the start word "
                "and/or target word from params.",
                "Missing puzzle metadata: start word or target word not found."
            )

        # 标准化预测答案，转换为小写
        pred_path = [str(word).strip().lower() for word in pred_path if str(word).strip()]

        if len(pred_path) < 2:
            return (
                False,
                self._feedback(
                    "The extracted path is too short after normalization. "
                    "Please provide the full ladder from start to target.",
        ...
        actual_steps = len(pred_path) - 1
        if min_steps is not None and max_steps is not None:
            if not (min_steps <= actual_steps <= max_steps):
                return False, self._feedback(
                    f"The path uses {actual_steps} steps, but the allowed range is "
                    f"{min_steps} to {max_steps} steps.",
                    "Invalid path length: step count out of allowed range."
                )

        # 1. 验证路径起点和终点
        if pred_path[0] != start_word:
            return False, self._feedback(
                f"The path must start with '{start_word}', but it starts with "
                f"'{pred_path[0]}'.",
                "Invalid path: wrong start word."
            )

        if pred_path[-1] != target_word:
            return False, self._feedback(
                f"The path must end with '{target_word}', but it ends with "
                f"'{pred_path[-1]}'.",
                "Invalid path: wrong target word."
            )
        ...
        for i in range(len(pred_path) - 1):
            ...
            if len(word1) != len(word2):
                return False, self._feedback(
                    f"Invalid transition at step {i + 1}: '{word1}' -> '{word2}'. "
                    "Word lengths differ, but Word Ladder moves must keep the "
                    "same length.",
                    f"Invalid transition at step {i + 1}: word lengths differ."
                )
            ...
            if diff_count != 1:
                return False, self._feedback(
                    f"Invalid transition at step {i + 1}: '{word1}' -> '{word2}'. "
                    f"These words differ by {diff_count} letters; each move must "
                    "change exactly one letter.",
                    f"Invalid transition at step {i + 1}: must change exactly one letter."
                )
        ...
        for word in pred_path:
            if word in seen:
                return False, self._feedback(
                    f"The path contains a repeated word: '{word}'. "
                    "A valid ladder should not repeat words.",
                    "Invalid path: repeated word."
                )
            seen.add(word)

        # 4. 检查每个单词是否是有效英文单词
        invalid_words = [word for word in pred_path if not self._is_valid_word(word)]
        if invalid_words:
            quoted_words = ", ".join(f"'{word}'" for word in invalid_words)
            return False, self._feedback(
                f"The following words are not recognized as valid English words "
                f"in the evaluator dictionary: {quoted_words}.",
                "Invalid path: contains non-dictionary word(s)."
            )

        # 所有检查都通过，解答正确
        return True, self._feedback(
            f"Correct Word Ladder. The path starts at '{start_word}', ends at "
            f"'{target_word}', changes exactly one letter per step, uses valid "
            f"English words, and contains {actual_steps} steps.",
            "Correct Word Ladder."
        )
